...
import { Token, TokenType, validComparators, validKeywords, validOperators, validSeparators } from "./token";
...
export function lex(file: string, reader: StringReader): LexReader {
  const tokens: Token[] = [];
  ...
  return new LexReader(file, tokens);
}
