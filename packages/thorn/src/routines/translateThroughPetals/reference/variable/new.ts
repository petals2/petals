...
export class NewResultReference extends VariableReference {
  ...
  performSideEffects(target: Target, thread: Block<string>, context: Context): void {
    // do the funny malloc
    thread.getTail().append(target.getBlocks().createBlock(Blocks.Procedures.Call, context.getHeap(this.type.getHeapName()).malloc.getPrototype(), undefined, Input.shadowed(new NumberInput(StructTool.getSize(this.type.dereference())))));
    ...
    if (def) {
      ...
      let call = target.getBlocks().createBlock(Blocks.Procedures.Call, def.getPrototype(), undefined, ...argValues);
      ...
      args.forEach((v, i) => {
        if (v instanceof VariableHeapCopyReference) {
          ...
          call = call.getTail().append(target.getBlocks().createBlock(Blocks.Procedures.Call, heap.free.getPrototype(), undefined, argValues[i]));
        }
      })
